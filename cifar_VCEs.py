...
from utils_svces.datasets.cifar import get_CIFAR10_labels
...
in_labels = get_CIFAR10_labels()
in_loader = dl.get_CIFAR10(train=False, augm_type='big_transfer_128', size=img_size)
...
for method in [hps.method]:
    ...
    with torch.no_grad():
        ...
        model_bs = bs
        dir = f'{out_dir}/{cifar_mode}/{norm}_{hps.l2_sim_lambda}_l1_{hps.l1_sim_lambda}_l{hps.lp_custom}_{hps.lp_custom_value}_classifier_{hps.classifier_type}_{hps.second_classifier_type}_{hps.third_classifier_type}_{hps.classifier_lambda}_reg_lpips_{hps.lpips_sim_lambda}_example_{hps.timestep_respacing}_steps_skip_{hps.skip_timesteps}_start_{hps.gen_type}_deg_{str(hps.deg_cone_projection)}_s_{str(hps.seed)}{"_bl" if hps.use_blended else ""}_wid_{hps.world_id}_{hps.world_size}_{hps.method}/'
        ...
        for batch_idx in trange(n_batches, desc=f'Batches progress'):
            ...
            with torch.no_grad():
                ...
                summarized_confidences = torch.zeros(orig_confidences.size(0), 10)  # Assuming orig_confidences has shape [batch_size, 1000]
                for i in range(10):
                    summarized_confidences[:, i] = orig_confidences[:, i*100:(i+1)*100].sum(dim=1)
            ...
            for radius_idx in range(len(radii)):
                ...
                summarized_batch_probs_i = torch.zeros(batch_probs_i.size(0), 10)  # Assuming orig_confidences has shape [batch_size, 1000]
                for i in range(10):
                    summarized_batch_probs_i[:, i] = batch_probs_i[:, i*100:(i+1)*100].sum(dim=1)

                batch_probs_in_i = torch.softmax(batch_model_in_i, dim=1)
                summarized_batch_probs_in_i = torch.zeros(batch_probs_in_i.size(0), 10)  # Assuming orig_confidences has shape [batch_size, 1000]
                for i in range(10):
                    summarized_batch_probs_in_i[:, i] = batch_probs_in_i[:, i*100:(i+1)*100].sum(dim=1)
