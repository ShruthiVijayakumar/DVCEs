...
from utils_svces.datasets.paths import get_CIFAR10_path
from utils_svces.datasets.celebA import celebA_attributes
...
#in_labels = get_CIFAR10_labels(hps.data_folder)
in_labels = celebA_attributes
desired_attributes = ["Brown_Hair"]
in_loader = dl.celebA_feature_set(split='test', augm_type='madry',attributes=desired_attributes, out_size=img_size)
...
for method in [hps.method]:
    ...
    with torch.no_grad():
        ...
        model_bs = bs
        dir = f'{out_dir}/{celebA_mode}/{norm}_{hps.l2_sim_lambda}_l1_{hps.l1_sim_lambda}_l{hps.lp_custom}_{hps.lp_custom_value}_classifier_{hps.classifier_type}_{hps.second_classifier_type}_{hps.third_classifier_type}_{hps.classifier_lambda}_reg_lpips_{hps.lpips_sim_lambda}_example_{hps.timestep_respacing}_steps_skip_{hps.skip_timesteps}_start_{hps.gen_type}_deg_{str(hps.deg_cone_projection)}_s_{str(hps.seed)}{"_bl" if hps.use_blended else ""}_wid_{hps.world_id}_{hps.world_size}_{hps.method}/'
        ...
        for batch_idx in trange(n_batches, desc=f'Batches progress'):
            ...
            with torch.no_grad():
                ...
                summarized_confidences = torch.zeros(orig_confidences.size(0), 40)  # Assuming orig_confidences has shape [batch_size, 1000]
                for i in range(40):
                    summarized_confidences[:, i] = orig_confidences[:, i*25:(i+1)*25].sum(dim=1)
                model_original_probabilities[batch_start_idx:batch_end_idx, :] = summarized_confidences.detach().cpu()
            ...
            for radius_idx in range(len(radii)):
                ...
                summarized_batch_probs_i = torch.zeros(batch_probs_i.size(0), 40)  # Assuming orig_confidences has shape [batch_size, 1000]
                for i in range(40):
                    summarized_batch_probs_i[:, i] = batch_probs_i[:, i*25:(i+1)*25].sum(dim=1)

                batch_probs_in_i = torch.softmax(batch_model_in_i, dim=1)
                summarized_batch_probs_in_i = torch.zeros(batch_probs_in_i.size(0), 40)  # Assuming orig_confidences has shape [batch_size, 1000]
                for i in range(40):
                    summarized_batch_probs_in_i[:, i] = batch_probs_in_i[:, i*25:(i+1)*25].sum(dim=1)


                out_probabilities[batch_start_idx:batch_end_idx, target_idx, radius_idx,
                :] = summarized_batch_probs_i.cpu().detach()
                in_probabilities[batch_start_idx:batch_end_idx, target_idx, radius_idx,
                :] = summarized_batch_probs_in_i.cpu().detach()
